...
ZigbeeAdapter::ZigbeeAdapter(std::chrono::seconds interval)
    : interval_(interval) {}
...
void ZigbeeAdapter::start(EventCallback on_event) {
...
void ZigbeeAdapter::publish_device_event(const DeviceEvent& event) {
    if (rabbitmq_client_) {
